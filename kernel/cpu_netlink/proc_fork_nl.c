...
#ifdef CONFIG_TCT_RTG_SCHED
#include "tctrtg/proc_state.h"
#endif
...
#if (defined(CONFIG_TCT_VIP_THREAD) && !defined(CONFIG_DISABLE_VIP_FORK_MSG))
	rcu_read_lock();
	do {
		if (pid != tgid) {
...
#if (defined(CONFIG_TCT_RTG_SCHED) )
