...
static int _iris_configure(u32 type, u32 value)
{
	...
	IRIS_LOGD("%s(), type: 0x%04x(%d), value: %d", __func__, type, type, value);
...
#ifdef CONFIG_PXLW_IRIS6_TWO_MCF
		if ((value > 0) && (value != pqlt_cur_setting->pq_setting.cmcolorgamut)) {
			u8 lut_type = CM_LUT;
			u8 lut_gam = (value) & 0x0f;
			u8 lut_opt = (lut_gam) * 0x10;
			u8 i = 0;

			for (i = 0; i < SUPPORT_2NITS; i++) {
				iris_update_ip_opt(lut_type,  (lut_opt+i+1), 0x1);
				IRIS_LOGI("update ip 3d lut: 0x%x", (lut_opt+i+1));
			}
			iris_init_update_ipopt_t(IRIS_IP_DMA, 0xe3, 0xe3, 0);
			iris_update_pq_opt(true);
		}
		if ((value > 0) && (pqlt_cur_setting->pq_setting.cmcolorgamut == 0)) {
			u32 bl_lvl = pqlt_cur_setting->system_brightness;

			iris_3dlut_interpolation_adjust(bl_lvl, true);
 
		}
		pqlt_cur_setting->pq_setting.cmcolorgamut = value;
		IRIS_LOGI("now cmcolorgamut is: 0x%x", value);
#else
		pqlt_cur_setting->pq_setting.cmcolorgamut = value;
...
#ifdef CONFIG_PXLW_IRIS6_TWO_MCF
				u8 lut_index = 0x11; //17-bin 3d-lut, 6 tables
				u8 lut_type = CM_LUT;
				u8 lut_gam = pqlt_cur_setting->pq_setting.cmcolorgamut;
#else
...
#ifdef CONFIG_PXLW_IRIS6_TWO_MCF
				lut_index = (lut_gam)*0x10;
				if (pqlt_cur_setting->pq_setting.cmcolorgamut == 0)
					lut_index = 0x10;

				for (i = 0; i < SUPPORT_2NITS; i++)
					iris_update_ip_opt(lut_type,  (lut_index+i+1), 0x1);
	...
	case IRIS_SDR2HDR_AI_SWITCH:
		...
		break;
	case IRIS_HDR_ENHANCE:
		pqlt_cur_setting->hdr_enahnce = value;
		iris_hdr_enhance_set(value, true);
		break;
	case IRIS_SDR2HDR_CAMERA:
		pqlt_cur_setting->sdr2hdr_camera = value;
		iris_sdr2hdr_camera(value);
		break;
	default:
		goto error;
	}
...
int iris_configure_ex(u32 type, u32 count, u32 *values)
{
	...
	case IRIS_CONTRAST_DIMMING:
		if (pcfg->dc_bl_pending != 1) {
			if (pqlt_cur_setting->dimmingEnable && (values[4] == 0))
				pcfg->dimming_first = 0;
			else if ((pqlt_cur_setting->dimmingEnable == 0) && values[4])
				pcfg->dimming_first = 1;
			else {
				if ((values[1] >  pcfg->pend_dimming[1])
					&& (values[3] < pqlt_cur_setting->system_brightness))
					pcfg->dimming_first = 0;
				else
					pcfg->dimming_first = 1;
			}
			...
			IRIS_LOGI("new dimming: 0x%x, bl: %d", values[1], values[3]);
...
int iris_configure_get(u32 type, u32 count, u32 *values)
{
	...
	case IRIS_SDR2HDR_AI_SWITCH:
		*values = pqlt_cur_setting->ai_kb_switch;
		break;
	case IRIS_HDR_ENHANCE:
		*values = pqlt_cur_setting->hdr_enahnce;
		break;
	case IRIS_SDR2HDR_CAMERA:
		*values = pqlt_cur_setting->sdr2hdr_camera;
		break;
	case IRIS_SDR2HDR_AI_INPUT_BACKLIGHT:
	case IRIS_SDR2HDR_AI_CTRL_TM_Y:
	case IRIS_SDR2HDR_AI_CTRL_TM_K:
