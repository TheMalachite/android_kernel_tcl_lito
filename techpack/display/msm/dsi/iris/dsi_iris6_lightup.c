...
extern char iris_module_name[256];
#endif
...
int iris_parse_param(struct device_node *np, struct dsi_panel *panel)
{
	...
	lightup_node = of_parse_phandle(np, "pxlw,iris-lightup-config", 0);
	if (!lightup_node) {
...
int iris_enable_from_lp(struct dsi_panel *panel)
{
	int rc = 0;
	struct iris_cfg *pcfg = iris_get_cfg();
	int lightup_opt = iris_lightup_opt_get();
	...
	while (1) {
		...
		rc = iris_lightup_exit_abyp(true, false);
...
static int iris_dev_probe(struct platform_device *pdev)
{
	...
	snprintf(iris_module_name, 256, "IRIS6:CSOT:V03:na"); //Inparm: V03
#else
	snprintf(iris_module_name, 256, "IRIS6:CSOT:V05:na"); //Inparm: V05
#endif
#endif
	return 0;
}
