		...
		struct dsi_cmd_desc **ptx_cmds, struct iris_ocp_cmd **pocp_cmds)
{
	...
	*ptx_cmds = (struct dsi_cmd_desc *) kvmalloc(cmds_cnt *sizeof(**ptx_cmds), GFP_KERNEL);
	if (!(*ptx_cmds)) {
		IRIS_LOGE("can not kvmalloc len = %lu\n", cmds_cnt * sizeof(**ptx_cmds));
		return -ENOMEM;
	}

	*pocp_cmds = (struct iris_ocp_cmd *)kvmalloc(cmds_cnt * sizeof(**pocp_cmds), GFP_KERNEL);
	if (!(*pocp_cmds)) {
		IRIS_LOGE("can not kvmalloc pocp cmds\n");
		kvfree(*ptx_cmds);
		*ptx_cmds = NULL;
		return -ENOMEM;
	}
