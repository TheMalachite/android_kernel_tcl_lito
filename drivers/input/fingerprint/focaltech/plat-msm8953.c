...
int ff_ctl_free_pins(void)
{
    ...
    gpio_free(g_config->gpio_power_pin); //[TCTNB][Focal-FP] MODIFIED-BEGIN by Ji.Chen, release power pin resource
...
int ff_ctl_reset_device(void)
{
    ...
    /* 3-2: Delay for 2ms. */
    mdelay(2); //[TCTNB][FOCAL-FP]MODIFIED-BEGIN by Ji.Chen,suggest from FAE
