...
/*[TCTNB][Focal-FP] MODIFIED-BEGIN by Ji.Chen, report finger event to HAL */
/*
 * TODO: Alias of ff_ctl_touch_event_callback, change to be the ODM defined name.
 */
//#define ff_ctl_touch_event_callback ff_ctl_touch_event_cb

/*
 * The touching event callback for In/Under-Display Fingerprint solution.
 * The TouchPanel driver should trigger this callback if the fingerprint hotzone
 * gets events.
 *
 * @params
 * event: The touching event code: 0 - TOUCH_UP, 1 - TOUCH_DOWN.
 *
 * @return
 *  0 if kobject_uevent_env() is completed with success or the corresponding error
 *  when it fails.
 */
int ff_ctl_touch_event_callback(uint8_t event) {
    int err = 0;
    FF_LOGV("'%s' enter.", __func__);

    if (likely(g_context) && likely(g_context->miscdev.this_device)) {
        char *uevent_env[2];
        switch (event) {
        case 1:
            uevent_env[0] = "FF_TOUCH_DOWN";
            break;
        default:
            uevent_env[0] = "FF_TOUCH_UP";
            break;
        }
        uevent_env[1] = NULL;
        err = kobject_uevent_env(&g_context->miscdev.this_device->kobj,
...
EXPORT_SYMBOL(ff_ctl_touch_event_callback);

/*[TCTNB][Focal-FP] MODIFIED-END by Ji.Chen, report finger event to HAL */
...
/* MODIFIED-END by ji.chen,BUG-7974483*/
    ...
    //ff_spi_unregister_device(); //[TCTNB][Focal-FP] MODIFIED-BEGIN by Ji.Chen
...
/* BUGFIX-MODIFIED-BEGIN by TCTNB.ji.chen,2020-03-26,PR-9046813*/
...
static long ff_ctl_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
    ...
    case FF_IOC_SYNC_CONFIG: {
        ff_driver_config_t _driver_config = driver_config; //[TCTNB][Focal-FP]MODIFIED-BEGIN by Ji.Chen
        if (copy_from_user(&driver_config, (ff_driver_config_t *)arg, sizeof(ff_driver_config_t))) {
            FF_LOGE("copy_from_user(..) failed.");
            err = (-EFAULT);
            break;
        }
        //[TCTNB][Focal-FP]MODIFIED-BEGIN by Ji.Chen
        FF_LOGD("sync gpio configs to ff_driver_config struct.");
        driver_config.gpio_mosi_pin = _driver_config.gpio_mosi_pin;
        driver_config.gpio_miso_pin = _driver_config.gpio_miso_pin;
        driver_config.gpio_ck_pin = _driver_config.gpio_ck_pin;
        driver_config.gpio_cs_pin = _driver_config.gpio_cs_pin;
        driver_config.gpio_rst_pin = _driver_config.gpio_rst_pin;
        driver_config.gpio_int_pin = _driver_config.gpio_int_pin;
        driver_config.gpio_power_pin = _driver_config.gpio_power_pin;
        driver_config.gpio_iovcc_pin = _driver_config.gpio_iovcc_pin;
        //[TCTNB][Focal-FP]MODIFIED-END by Ji.Chen
...
//[TCTNB][FOCAL-FP]MODIFIED-BEGIN by Ji.Chen, Fix fp module info Compatibility issue
    case FF_IOC_SET_VENDOR_INFO: {
        const __u8 *encoded_dat = (const __u8 *)arg;
        const __u32 vendor_info_size = 32;
        char vendor_info[vendor_info_size];

        /* Decode the length of vendor info. */
        __u32 vendor_info_len = 0;
        if (copy_from_user(&vendor_info_len, encoded_dat, sizeof(__u32))) {
            FF_LOGE("copy_from_user(..) failed.");
            err = (-EFAULT);
            break;
        }
        if (vendor_info_len > vendor_info_size) {
            FF_LOGE("vendor info is too long: %u > %u.", vendor_info_len, vendor_info_size);
            err = (-EOVERFLOW);
            break;
        }

        /* Copy the vendor info (includeing the terminating null character). */
        encoded_dat += sizeof(__u32);
        if (copy_from_user(vendor_info, encoded_dat, vendor_info_len)) {
            FF_LOGE("copy_from_user(..) failed.");
            err = (-EFAULT);
            break;
        }

        FF_LOGD("vendor_info: '%s'.", vendor_info);
#if defined CONFIG_TCT_DEVICEINFO
        strcpy(fp_module_name, vendor_info);
#endif
        break;
    }
//[TCTNB][FOCAL-FP]MODIFIED-END by Ji.Chen, Fix fp module info Compatibility issue
    default:
        err = (-EINVAL);
        break;
    }
...
/* BUGFIX-MODIFIED-BEGIN by TCTNB.ji.chen,2020-03-26,PR-9046813*/
#if defined CONFIG_TCT_DEVICEINFO
			//sprintf(fp_module_name,"FT9607:FOCAL:SUNWIN");
