...
#define	GF_INPUT_NAME	    "gw9558"	/*"goodix_fp" */
...
/* BUGFIX-MODIFIED-BEGIN by TCTNB.ji.chen,2020-03-26,PR-9046813*/
#if defined CONFIG_TCT_DEVICEINFO
        sprintf(fp_module_name,"GW9558:GOODIX:A-KERR");
#endif
/* BUGFIX-MODIFIED-End by TCTNB.ji.chen*/
	...
	if (status == 0) {
		if (status == 0) {
			...
			if (gf_dev->users == 1) {
				...
				msleep(10); // MODIFIED by ji.chen, 2020-03-28,BUG-9030639
			}
			gf_hw_reset(gf_dev, 3);
			gf_dev->device_available = 1;
		}
...
/* MODIFIED-BEGIN by ji.chen, 2020-03-31,BUG-9145997*/
int gf_touch_event_handler(uint8_t touch_state)
{
    static uint8_t gf_last_touch_state = 0x00;
    char fp_down_msg = GF_NET_EVENT_FINGER_DOWN;
    char fp_up_msg = GF_NET_EVENT_FINGER_UP;

    pr_info("[info] %s touch_state %d\n", __func__, touch_state);

#if 0
    if(tp_info->touch_state == gf_last_touch_state) {
        return 0;
    }
#endif
    ...
    wake_lock_timeout(&fp_wakelock, msecs_to_jiffies(WAKELOCK_HOLD_TIME));
    if (0x01 == touch_state) {
        pr_info("[info] %s touch finger down\n", __func__);
#if defined(GF_NETLINK_ENABLE)
        sendnlmsg(&fp_down_msg);
#endif
    } else {
        pr_info("[info] %s touch finger up\n", __func__);
#if defined(GF_NETLINK_ENABLE)
        sendnlmsg(&fp_up_msg);
#endif
    }
    gf_last_touch_state = touch_state;

    return 0;
}
EXPORT_SYMBOL(gf_touch_event_handler);
/* MODIFIED-END by ji.chen,BUG-9145997*/
