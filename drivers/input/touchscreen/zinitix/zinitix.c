...
#define ZINITIX_SPEED_UP 1
extern char *saved_command_line;

#define KEY_GESTURE_DOUBLETAP      237
bool bt541_gesture_enable = false;
...
#define PINCTRL_STATE_ACTIVE	"pmx_sub_ts_active"
...
#define TOUCH_CP_MODE_Y         12
#define TOUCH_CP_MODE_X         13
...
struct tsp_raw_data {
	...
	s16 short_data[TSP_CMD_NODE_NUM];
	s16 cp_y_data[10];
	s16 cp_x_data[10];
};
...
struct bt541_ts_info {
	...
	bool device_enabled;
	bool checkUMSmode;
	bool rawdata_test_flag;
	bool short_test_flag;
	bool cp_test_flag;
...
static bool zinitix_get_raw_data(struct bt541_ts_info *info, u8 *buff, u16 len, int skip_cnt)
{
	struct i2c_client *client = info->client;
	struct bt541_ts_platform_data *pdata = info->pdata;
	int i;
	...
	if (read_raw_data(client, BT541_RAWDATA_REG, (char *)(buff), len) < 0) {
		dev_err(&client->dev, "%s: Failed to read short data\n", __func__);
		info->work_state = NOTHING;
		enable_irq(info->irq);
		up(&info->work_lock);
...
retry_upgrade:
	...
	if (write_cmd(client, 0x01d5) != I2C_SUCCESS) {
		zinitix_printk("enter upgrade mode failed\n");
		goto fail_upgrade;
	}

	usleep_range(1*1500, 1*1500);
...
extern char sub_ctp_module_name[256];
#endif
...
//	cap->ic_fw_size = 44*1024;
	...
	pr_info("bt541_ts: %s, fw_version=0x%x, fw_minor_version=0x%x, reg_data_version=0x%x", 
...
#ifdef CONFIG_TCT_DEVICEINFO
    sprintf(sub_ctp_module_name, "ZINITIX:TRULY:0x%x:NA",
	((info->cap_info.fw_version & 0xf) << 12) | \
...
static irqreturn_t bt541_touch_work(int irq, void *data)
{
	...
	if((tpd_halt) && (info->work_state == SUSPEND) && (bt541_gesture_enable == true)) { //suspend state support gesture
		...
		if (read_data(client, 0x126,(u8 *)&gesture_flag, 2) < 0) {//0x126 
			...
			if (gesture_flag == 2) { /*1 double click*/
				input_report_key(info->input_dev, KEY_GESTURE_DOUBLETAP, 1);
				input_sync(info->input_dev);
				input_report_key(info->input_dev, KEY_GESTURE_DOUBLETAP, 0);
				input_sync(info->input_dev);
...
#if ZINITIX_SPEED_UP
static void bt541_resume_work(struct work_struct *work)
{
	struct bt541_ts_info *info =
			container_of(work, struct bt541_ts_info, resume_work);
	struct i2c_client *client = info->client;

    bt541_ts_resume(&client->dev);
}
#endif
...
static int drm_check_dt(struct device_node *np)
{
    ...
    if (count <= 0) {
		pr_err("%s: find drm_panel count(%d) fail\n", __func__, count);
        return -ENODEV;
    }
                                 ...
                                 unsigned long event, void *data)
{
	...
	if (evdata && evdata->data && event == DRM_PANEL_EVENT_BLANK &&
		...
		pr_info("[bt541]: %s, event=%d, blank=%d", __func__, event, *blank);
...
static bool ts_set_touchmode(u16 value)
{
	...
	if (value == TOUCH_PDND_MODE) {
		...
		if (write_reg(misc_info->client, BT541_AFE_FREQUENCY,
			...
			printk(KERN_INFO "[zinitix_touch] TEST Mode : "
					"Fail to set BT541_AFE_FREQUENCY %d.\n", SEC_PDND_FREQUENCY);
	} else if (value == TOUCH_CP_MODE_Y || value == TOUCH_CP_MODE_X) {
		if (write_reg(misc_info->client, BT541_DND_N_COUNT,
			SEC_CP_N_COUNT) != I2C_SUCCESS)
			printk(KERN_INFO "[zinitix_touch] TEST Mode : "
					"Fail to set BT541_DND_N_COUNT %d.\n", SEC_CP_N_COUNT);
		if (write_reg(misc_info->client, BT541_DND_U_COUNT,
			SEC_CP_U_COUNT) != I2C_SUCCESS)
			printk(KERN_INFO "[zinitix_touch] TEST Mode : "
					"Fail to set BT541_DND_U_COUNT %d.\n", SEC_CP_U_COUNT);
		if (write_reg(misc_info->client, BT541_AFE_FREQUENCY,
			SEC_CP_FREQUENCY) != I2C_SUCCESS)
			printk(KERN_INFO "[zinitix_touch] TEST Mode : "
					"Fail to set BT541_AFE_FREQUENCY %d.\n", SEC_CP_FREQUENCY);
	...
	if (misc_info->touch_mode == TOUCH_PDND_MODE) {
	...
	if (write_cmd(misc_info->client, BT541_SWRESET_CMD) != I2C_SUCCESS) {
		pr_err("%s: Failed to write reset command\n", __func__);
		//goto fail_init;
	}
	mdelay(400);
...
#define BT541_CSV_FILE_NAME                       "_testdata.txt"
#define BT541_INI_FILE_PATH                       "/mnt/sdcard/"
#define BT541_RAWDATA_RESULT                  "rawdata/"
#define BT541_TEST_RESULT_PACH                    "/mnt/sdcard/rawdata/"
...
static int bt541_test_save_test_data(char *file_name, char *data_buf, int len)
{
	...
	if (misc_info->rawdata_test_flag && misc_info->short_test_flag)
		snprintf(filepath, FILE_NAME_LENGTH, "%s%s%s%d%s",
			BT541_INI_FILE_PATH, BT541_RAWDATA_RESULT, "SUB_TP_PASS_", rawdata_begin_time.tv_sec,file_name);
	else
		snprintf(filepath, FILE_NAME_LENGTH, "%s%s%s%d%s",
			BT541_INI_FILE_PATH, BT541_RAWDATA_RESULT, "SUB_TP_FAILED_", rawdata_begin_time.tv_sec,file_name);

    if (NULL == pfile) {
        pfile = filp_open(filepath, O_TRUNC | O_CREAT | O_RDWR, 0);
    }
    if (IS_ERR(pfile)) {
        pr_err("%s, error occured while opening file %s.", __func__, filepath);
        return -EIO;
    }
...
static void bt541_test_save_data_csv(struct bt541_ts_info *info)
{
	int i = 0;
    int j = 0;
	int data_count = 0;
    ...
    pr_info("%s, save data in csv format", __func__);
    csv_buffer = vmalloc(CSV_BUFFER_LEN);
    if (!csv_buffer) {
        pr_err("%s, csv_buffer malloc fail\n", __func__);
        return;
    }
    ...
    csv_length += snprintf(csv_buffer + csv_length, \
                           ...
                           "Rawdata Test:%s\n", \
						   info->rawdata_test_flag?"PASS":"FAILED");

	/* line 2 */
	for (i = 0; i < info->cap_info.y_node_num; i++) {
		for (j = 0; j < info->cap_info.x_node_num; j++) {
			data_count = i * info->cap_info.x_node_num + j;
			csv_length += snprintf(csv_buffer + csv_length, \
                           CSV_BUFFER_LEN - csv_length, \
                           "%d ", raw_data->pref_data[data_count]);
		}
		csv_length += snprintf(csv_buffer + csv_length, \
				CSV_BUFFER_LEN - csv_length, \
				"\n");
    ...
    csv_length += snprintf(csv_buffer + csv_length, \
                           CSV_BUFFER_LEN - csv_length, \
                           "Short Test:%s\n", \
						   info->short_test_flag?"PASS":"FAILED");

	for (i = 0; i < (CHECK_SHORT_DATA_NUM>>1); i++) {
		csv_length += snprintf(csv_buffer + csv_length, \
							CSV_BUFFER_LEN - csv_length, \
							"%d ", raw_data->short_data[i]);
	}
	...
	for (i = 0; i < 10; i++) {
		csv_length += snprintf(csv_buffer + csv_length, \
							CSV_BUFFER_LEN - csv_length, \
							"%d ", raw_data->cp_x_data[i]);
	}
	csv_length += snprintf(csv_buffer + csv_length, \
						CSV_BUFFER_LEN - csv_length, \
						"\n\n");

    bt541_test_save_test_data(BT541_CSV_FILE_NAME, csv_buffer, csv_length);
...
static int zinitix_rawdata_test(struct bt541_ts_info *info)
{
	struct tsp_raw_data *raw_data = info->raw_data;
	...
	for (i = 0; i < (info->cap_info.x_node_num * info->cap_info.y_node_num); i++) {
		if(((raw_data->pref_data[i]) < min[i]) || ((raw_data->pref_data[i]) > max[i])) {
			pr_info("[bt541]%s: test falied raw_data[%d]=%d, min=%d, max=%d",
				__func__, i, raw_data->pref_data[i], min[i], max[i]);
			return false;
		} else {
			continue;
		}
...
static int zinitix_short_test(struct bt541_ts_info *info)
{
	struct tsp_raw_data *raw_data = info->raw_data;
    const u16 NORMAL_SHORT_VALUE = 1000;
	u16 i;
	...
	ts_set_touchmode(TOUCH_CHECK_SHORT_MODE);
	get_short_data(info, (u8 *)raw_data->short_data, 10);
	ts_set_touchmode(TOUCH_POINT_MODE);

	for (i = 0; i < (CHECK_SHORT_DATA_NUM>>1); i++) {
		if (raw_data->short_data[i] != NORMAL_SHORT_VALUE) {
			pr_info("[bt541]Invalid Check Short Value, Test Short! short_data[%d]=%d\n",
				i, raw_data->short_data[i]);
			return false;
		}
	}
...
#define ZTW622_MAX_CHANNEL_SIZE           20  //bytes

static int zinitix_cp_test(struct bt541_ts_info *info)
{
	struct tsp_raw_data *raw_data = info->raw_data;

	ts_set_touchmode(TOUCH_CP_MODE_Y);
	zinitix_get_raw_data(info, (u8 *)raw_data->cp_y_data, ZTW622_MAX_CHANNEL_SIZE, 10);
	ts_set_touchmode(TOUCH_CP_MODE_X);
	zinitix_get_raw_data(info, (u8 *)raw_data->cp_x_data, ZTW622_MAX_CHANNEL_SIZE, 10);
	ts_set_touchmode(TOUCH_POINT_MODE);

	return true;
}

static ssize_t show_factory_test(struct device *dev, struct device_attribute
				    *devattr, char *buf)
{
	struct bt541_ts_info *info = dev_get_drvdata(dev);

	pr_info("bt541 factory test start");

	do_gettimeofday(&rawdata_begin_time);
	info->rawdata_test_flag = zinitix_rawdata_test(info);
	info->short_test_flag = zinitix_short_test(info);
	zinitix_cp_test(info);
	zinitix_hw_reset(info, true);

	if ((dev_mkdir(BT541_TEST_RESULT_PACH, S_IRUGO | S_IWUSR)) != 0)
        pr_err("%s, Failed to create directory for tp test", __func__);

	bt541_test_save_data_csv(info);

	if (info->rawdata_test_flag && info->short_test_flag)
...
static DEVICE_ATTR(factory_test, S_IRUGO, show_factory_test, NULL);
...
static int zinitix_hw_reset(struct bt541_ts_info* data, bool on )
{
	...
	if (on) {
		err = write_cmd(client, 0x0FF8);
		if (err != I2C_SUCCESS)
			dev_err(&client->dev, "Faied to send 0x0FF8 CMD\n");
#if 0
		...
		const struct i2c_device_id *i2c_id)
{
...
#if defined(CONFIG_DRM)
#if defined(CONFIG_DRM_PANEL)
	ret = drm_check_dt(client->dev.of_node);
	if (ret)
		pr_err("%s: parse drm-panel fail", __func__);
#endif
#endif
	...
	input_set_capability(info->input_dev, EV_KEY, KEY_GESTURE_DOUBLETAP);
	__set_bit(KEY_GESTURE_DOUBLETAP, info->input_dev->keybit);
	dev_err(&client->dev,"KEY_GESTURE_DOUBLETAP register finish\n");
...
#if ZINITIX_SPEED_UP
    info->ts_workqueue = create_singlethread_workqueue("bt541_wq");
    if (!info->ts_workqueue) {
        dev_err(&client->dev, "create BT541 workqueue fail");
    }

	if (info->ts_workqueue) {
        INIT_WORK(&info->resume_work, bt541_resume_work);
    }
#endif
	/* configure irq */
...
#if defined(CONFIG_DRM)
    info->fb_notif.notifier_call = drm_notifier_callback;
#if defined(CONFIG_DRM_PANEL)
    if (active_panel) {
        ret = drm_panel_notifier_register(active_panel, &info->fb_notif);
        if (ret)
            dev_err(&info->client->dev,"[DRM]drm_panel_notifier_register fail: %d\n", ret);
    }
#else
    ret = msm_drm_register_client(&info->fb_notif);
    if (ret) {
        dev_err(&info->client->dev,"[DRM]Unable to register fb_notifier: %d\n", ret);
    }
#endif
#elif defined(CONFIG_FB)
