...
static void tct_touch_class_create(struct fts_ts_data *ts_data);
#define OFF_STATE 0
#define GESTURE_STATE 1
...
static int fts_read_touchdata(struct fts_ts_data *data)
{
    ...
    if (data->gesture_mode || data->fod_mode) {
        if (0 == tct_fts_gesture_readdata(data, NULL)) {
...
void tct_grip_mode_set(u8 mode)
{
    u8 buf[61] = { 0 };
    u8 edge_palm_state = 0;
    u8 game_mode = 0;
#ifdef CONFIG_TCT_TOUCH_GAME_MODE
    struct fts_ts_data *ts_data = fts_data;
#endif

    FTS_INFO("GRIP MODE:%d", mode);
    buf[0] = 0xC8;
    switch (mode) {
        case VERTICAL_DEFAULT:
            memcpy(&buf[1], vertical_default, 60);
            edge_palm_state = 0;
            game_mode = 0;
            break;
        case VERTICAL_NARROW:
            memcpy(&buf[1], vertical_narrow, 60);
            edge_palm_state = 0;
            game_mode = 0;
            break;
        case VERTICAL_WIDE:
            memcpy(&buf[1], vertical_wide, 60);
            edge_palm_state = 0;
            game_mode = 0;
            break;
        case HORIZONTAL_LEFT:
            memcpy(&buf[1], horizontal_left, 60);
            edge_palm_state = 1;
            game_mode = 0;
            break;
        case HORIZONTAL_LEFT_NARROW:
            memcpy(&buf[1], horizontal_left_narrow, 60);
            edge_palm_state = 1;
            game_mode = 0;
            break;
        case HORIZONTAL_LEFT_WIDE:
            memcpy(&buf[1], horizontal_left_wide, 60);
            edge_palm_state = 1;
            game_mode = 0;
            break;
        case HORIZONTAL_RIGHT:
            memcpy(&buf[1], horizontal_right, 60);
            edge_palm_state = 2;
            game_mode = 0;
            break;
        case HORIZONTAL_RIGHT_NARROW:
            memcpy(&buf[1], horizontal_right_narrow, 60);
            edge_palm_state = 2;
            game_mode = 0;
            break;
        case HORIZONTAL_RIGHT_WIDE:
            memcpy(&buf[1], horizontal_right_wide, 60);
            edge_palm_state = 2;
            game_mode = 0;
            break;
        case DIALER:
            memcpy(&buf[1], dialer, 60);
            edge_palm_state = 0;
            game_mode = 0;
            break;
        case GAME_HORIZONTAL_LEFT:
#ifdef CONFIG_TCT_TOUCH_GAME_MODE
            if (ts_data->game_grip_level == 2)
                memcpy(&buf[1], game_horizontal_left_high, 60);
            else if (ts_data->game_grip_level == 3)
                memcpy(&buf[1], game_horizontal_left_highest, 60);
            else
                memcpy(&buf[1], game_horizontal_left_regular, 60);
#else
            memcpy(&buf[1], game_horizontal_left, 60);
#endif
            edge_palm_state = 1;
            game_mode = 1;
            break;
        case GAME_HORIZONTAL_RIGHT:
#ifdef CONFIG_TCT_TOUCH_GAME_MODE
            if (ts_data->game_grip_level == 2)
                memcpy(&buf[1], game_horizontal_right_high, 60);
            else if (ts_data->game_grip_level == 3)
                memcpy(&buf[1], game_horizontal_right_highest, 60);
            else
                memcpy(&buf[1], game_horizontal_right_regular, 60);
#else
            memcpy(&buf[1], game_horizontal_right, 60);
#endif
            edge_palm_state = 2;
            game_mode = 1;
            break;
...
#ifdef CONFIG_TCT_TOUCH_GAME_MODE
            if (ts_data->game_grip_level == 2)
                memcpy(&buf[1], game_vertical_high, 60);
            else if (ts_data->game_grip_level == 3)
                memcpy(&buf[1], game_vertical_highest, 60);
            else
                memcpy(&buf[1], game_vertical_regular, 60);
#else
            memcpy(&buf[1], game_vertical, 60);
#endif
            edge_palm_state = 0;
            game_mode = 1;
            break;
    ...
    fts_write(buf, 61);
    fts_write_reg(0x8C, edge_palm_state);
    fts_write_reg(0xC3, game_mode);
}
...
static int fts_ts_probe_entry(struct fts_ts_data *ts_data)
{
    ...
    ts_data->tct_grip_mode = VERTICAL_DEFAULT;
#ifdef CONFIG_TCT_LITO_CHICAGO
    ts_data->angle_state = 3;
#endif
...
static int fts_ts_suspend(struct device *dev)
{
    ...
    if (ts_data->gesture_mode || ts_data->fod_mode) {
        fts_gesture_suspend(ts_data);
        ts_data->tp_suspend_state = GESTURE_STATE;
    } else {
        fts_irq_disable();
        ...
        ts_data->tp_suspend_state = OFF_STATE;
    }
...
static int fts_ts_resume(struct device *dev)
{
    ...
    //if (ts_data->gesture_mode || ts_data->fod_mode) {
    if (ts_data->tp_suspend_state == GESTURE_STATE) {
        fts_gesture_resume(ts_data);
    } else {
        fts_irq_enable();
    }

    tct_grip_mode_set(ts_data->tct_grip_mode);

    ts_data->suspended = false;
    ts_data->is_resuming = false;
    FTS_FUNC_EXIT();
    return 0;
}
...
extern int gf_touch_event_handler(uint8_t touch_state);
extern int ff_ctl_touch_event_callback(uint8_t touch_state); //[TCTNB][Focal-FP]MODIFIED-BEGIN by Ji.Chen
extern char fp_module_name[256];

static ssize_t op_display_notify_press_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return -EPERM;
}

static ssize_t op_display_notify_press_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    fp_name = strnstr(fp_module_name, "FT9607", strlen(fp_module_name));
    if (fp_name) {
            fp_id = 1;
            pr_info("%s: %s, val=%d\n", __func__, fp_name, val);
    }

    fp_name = NULL;

	fp_name = strnstr(fp_module_name, "GW9558", strlen(fp_module_name));
	if (fp_name) {
		fp_id = 2;
		pr_info("%s: %s, val=%d\n", __func__, fp_name, val);
	}
    ...
    if (fp_id == 1) {
        ff_ctl_touch_event_callback(val);
    } else if (fp_id == 2) {
        gf_touch_event_handler(val);
    } else {
        pr_err("%s: fp_name is error: %s", __func__, fp_module_name);
    }
...
static ssize_t tct_grip_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return scnprintf(buf, PAGE_SIZE, "%d\n", fts_data->tct_grip_mode);
}

static ssize_t tct_grip_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
    u8 val;
	struct input_dev *input_dev = fts_data->input_dev;
	...
	kstrtou8(buf, 0, &val);
	FTS_INFO("val=%d, tct_grip_mode=%d", val, fts_data->tct_grip_mode);
	fts_data->tct_grip_mode = val;
...
#ifdef CONFIG_TCT_LITO_CHICAGO
void tct_angle_state_set(struct fts_ts_data *ts_data)
{
    int ret = 0;
    u8 val = 0;

    val = ts_data->angle_state;
    ret = fts_write_reg(FTS_REG_ANGLE_STATE, val);
    if (ret < 0)
        FTS_ERROR("write FTS_REG_ANGLE_STATE failed ret=%d", ret);
}
static ssize_t tct_angle_state_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
    struct fts_ts_data *ts_data = fts_data;
    ...
    mutex_lock(&input_dev->mutex);
    ret = fts_read_reg(FTS_REG_ANGLE_STATE, &val);
    if (ret < 0)
        FTS_ERROR("read FTS_REG_ANGLE_STATE failed ret=%d", ret);
    else
        count += snprintf(buf + count, PAGE_SIZE, "Angle state Reg(0x9E):%d\n", val);
    count += snprintf(buf + count, PAGE_SIZE, "Angle state:%d\n", ts_data->angle_state);
    mutex_unlock(&input_dev->mutex);
    return count;
}

static ssize_t tct_angle_state_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
    u8 val;
    int ret = 0;
    ...
    if (val >= 0 && val <= 3) {
        FTS_INFO("val=%d", val);
        fts_data->angle_state = val;
        ret = fts_write_reg(FTS_REG_ANGLE_STATE, val);
        if (ret < 0)
            FTS_ERROR("write FTS_REG_ANGLE_STATE failed ret=%d", ret);
    } else {
        FTS_ERROR("wrong value: %d", val);
    }
    mutex_unlock(&input_dev->mutex);
...
#ifdef CONFIG_TCT_LITO_OTTAWA
static DEVICE_ATTR(op_display_notify_press, (S_IRUGO | S_IWUSR | S_IWGRP), 
                op_display_notify_press_show, op_display_notify_press_store);
#endif
static DEVICE_ATTR(grip_mode, (S_IRUGO | S_IWUSR | S_IWGRP), tct_grip_show, tct_grip_store);
#ifdef CONFIG_TCT_LITO_CHICAGO
static DEVICE_ATTR(angle_state, (S_IRUGO | S_IWUSR | S_IWGRP), tct_angle_state_show, tct_angle_state_store);
#endif
...
#ifdef CONFIG_TCT_LITO_OTTAWA
			error = device_create_file(ts_data->tct_touch_dev, &dev_attr_op_display_notify_press);
			if (error < 0)
                FTS_ERROR("Failed to create file op_display_notify_press!");
#endif
            error = device_create_file(ts_data->tct_touch_dev, &dev_attr_grip_mode);
			if (error < 0)
                FTS_ERROR("Failed to create file grip_mode!");
#ifdef CONFIG_TCT_LITO_CHICAGO
            error = device_create_file(ts_data->tct_touch_dev, &dev_attr_angle_state);
			if (error < 0)
                FTS_ERROR("Failed to create file angle_state!");
#endif
		} else
            FTS_ERROR("Failed to create tct_touch_dev!");
	}
	else
