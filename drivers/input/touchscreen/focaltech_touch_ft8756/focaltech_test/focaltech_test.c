...
*****************************************************************************/
//begin add by xiongbo.huang for Irvine Rawdata on 20200803
#include <linux/time.h> 
static int rawdata_test_flag = 0;//use fail for default
static struct timeval rawdata_begin_time;
//end add by xiongbo.huang for Irvine Rawdata on 20200803
...
//begin modified by xiongbo.huang for rawdata test on 20200803
    if(rawdata_test_flag){
        snprintf(filepath, FILE_NAME_LENGTH, "%s%s%s%d%s", 
            FTS_INI_FILE_PATH, TCT_FTS_RAWDATA_RESULT,"PASS_",rawdata_begin_time.tv_sec,file_name);
    }else{
        snprintf(filepath, FILE_NAME_LENGTH, "%s%s%s%d%s", 
            FTS_INI_FILE_PATH, TCT_FTS_RAWDATA_RESULT,"FAIL_",rawdata_begin_time.tv_sec,file_name);
    }
    
//end modified by xiongbo.huang for rawdata test on 20200803
    if (NULL == pfile) {
...
static int fts_test_main_exit(void)
{
    ...
    if ((dev_mkdir(FTS_TEST_RESULT_PACH, S_IRUGO | S_IWUSR)) != 0)
        FTS_TEST_ERROR("Failed to create directory for tp test");
    ...
    struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    snprintf(fwname, FILE_NAME_LENGTH, "%s", "Conf_MultipleTest.ini");
...
*/
//begin modfied by xiongbo.huang for irvine tp rawdata on 20200731
static DEVICE_ATTR(fts_test, S_IRUGO | S_IWUSR, fts_test_show, fts_test_store);
//static DEVICE_ATTR(fts_test, S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP | S_IROTH | S_IWOTH , fts_test_show, fts_test_store);
//end modfied by xiongbo.huang for irvine tp rawdata on 20200731
