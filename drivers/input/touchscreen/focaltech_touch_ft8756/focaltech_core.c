...
//begin modified by xiognbo.huang for irvine power off
#define INTERVAL_READ_REG                   100  /* unit:ms */
//end modified by xiognbo.huang for irvine power off
...
#endif
//end add by xiongbo.huang for irvine load tp firmware on 2020082
...
//begin add by xiongbo.huang for irvine sleep current on 20200828
#ifdef CONFIG_TCT_PROJECT_IRVINE
void fts_tp_reset(int status){
    if(fts_data->pdata->reset_gpio > 0){
        gpio_direction_output(fts_data->pdata->reset_gpio, status);
    }else{
        printk("fts_tp_reset set tp reset fail\n");
    }
    return;
}
#endif
//end add by xiongbo.huang for irvine sleep current on 20200828
...
static int fts_get_ic_information(struct fts_ts_data *ts_data)
{
    ...
    FTS_ERROR("fts_get_ic_information begin\n");
...
//begin modified by xiongbo.huang to delete log on 20200908
...
//end modified by xiongbo.huang to delete log on 20200908
#else
static int drm_notifier_callback(struct notifier_block *self,
                                 unsigned long event, void *data)
{
...
//begin add by xiongbo.huang to avoid screen-on no work add no use judge here
    ret = fts_wait_tp_to_valid();
    if(ret != 0){
        /* check if need recovery fw */
        FTS_ERROR("TP is not valid,download fw again\n");
        fts_reset_proc(60);
        fts_fw_recovery();
        ts_data->fw_is_running = true;
    }
//end add by xiongbo.huang on 20200819
    fts_ex_mode_recovery(ts_data);
#if FTS_ESDCHECK_EN
    fts_esdcheck_resume();
#endif
...
//begin modified by xiongbo.huang for Irvine tp suspend on 20200811
late_initcall(fts_ts_init);
//end modified by xiongbo.huang for Irvine tp suspend on 20200811
