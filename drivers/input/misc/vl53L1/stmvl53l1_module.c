...
// add by jliu on 2021/05/06, for read tof calibration data
...
{
	int rc;
	vl53l1_dbgmsg("force_device_on_en=%d\n", data->force_device_on_en);
	...
	if (!rc)
		data->reset_state = 1;
	// begin modified by jliu on 2021/04/25, don't power down if force_device_on_en is 1.
	vl53l1_dbgmsg("turn off vdd data reset_state=%d\n", data->reset_state);
	rc = stmvl53l1_module_func_tbl.power_down(data->client_object);
	// end modified by jliu on 2021/04/25, don't power down if force_device_on_en is 1.
...
// begin add by jliu on 2021/05/06, for read tof calibration data.
...
// golden calibration data, read from the golden vl53l1_cali.bin
static int cali_size = 372; // golden calibration data size
...
// end add by jliu on 2021/05/06, for read tof calibration data.
