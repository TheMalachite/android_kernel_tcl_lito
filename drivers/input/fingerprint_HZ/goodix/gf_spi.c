...
static void gf_kernel_key_input(struct gf_dev *gf_dev, struct gf_key *gf_key)
{
	...
	} else if (gf_key->key == GF_KEY_BACK) {//modefied by weiqian.zhou for defect 9796878 on 2020-09-03
        key_input = GF_KEY_INPUT_BACK;
    } else if(gf_key->key == GF_KEY_MENU) {
        key_input = GF_KEY_INPUT_MENU;
    } else {
	...
	if (gf_key->key == GF_KEY_BACK || gf_key->key == GF_KEY_MENU) {//modefied by weiqian.zhou for defect 9796878 on 2020-09-03
        input_report_key(gf_dev->input, key_input, 1);
		input_sync(gf_dev->input);
        input_report_key(gf_dev->input, key_input, 0);
        input_sync(gf_dev->input);
...
/*added by weiqiang.zhou for develop device info begin*/
#ifdef CONFIG_TCT_DEVICEINFO
    sprintf(fp_module_name,"GX3626:GOODIX:KERR:AYB0000204C1");
#endif
/*added by weiqiang.zhou for develop device info end*/
	return status;
err_irq:
	gf_cleanup(gf_dev);
err_parse_dt:
...
static int gf_release(struct inode *inode, struct file *filp)
{
	...
	if (!gf_dev->users) {
		...
		gf_disable_irq(gf_dev);
		//[Task-8825926]-ADD-BEGIN by TCTNB.Ji.Chen,2020/03/14,Fix fingerprint 2nd supplier issue
		if (gf_dev->irq) {
			disable_irq_wake(gf_dev->irq); //[TCTNB][GOODIX-FP]MODIFIED-BEGIN by Ji.Chen
			free_irq(gf_dev->irq, gf_dev);
		}
		...
		//[Task-8825926]-ADD-END by TCTNB.Ji.Chen,2020/03/14
	}
	mutex_unlock(&device_list_lock);
	return status;
}
...
/*Modefied by weiqiang.zhou ,FR  10301608 ,on 2020.11.10 Begin*/

#if defined(CONFIG_DRM)
#if defined(CONFIG_DRM_PANEL)
static int gf_check_panel_dt(struct gf_dev *chg)
{
	int i;
	int count;
	...
	if (!np) {
		pr_info("'%s' failed to find qcom,msm-notifier node.\n", __func__);
		return -ENODEV;
	}
    ...
    pr_info("'%s' failed to find panel.", __func__);
	return -ENODEV;
}
static int goodix_panel_notifier_callback(struct notifier_block *nb,
...
//	pr_info("[info] %s go to the goodix_panel_notifier_callback value = %d\n",
//			__func__, (int)event);
	...
	if (evdata && evdata->data &&  gf_dev) {
		...
		case DRM_PANEL_BLANK_POWERDOWN:
            if (DRM_PANEL_EARLY_EVENT_BLANK == event) {
                pr_info("resume: event = %lu, not care\n", event);
		...
		case DRM_PANEL_BLANK_UNBLANK:
            ...
            if (DRM_PANEL_EARLY_EVENT_BLANK == event) {
                ...
                pr_info("suspend: event = %lu, not care\n", event);
            }
			break;
		default:
			pr_info("%s defalut\n", __func__);
...
#else
int goodix_drm_state_chg_callback(struct notifier_block *nb,
				unsigned long event, void *data)
{
	...
	pr_info("[info] %s go to the goodix_drm_state_chg_callback value = %d\n",
			__func__, (int)event);
	gf_dev = container_of(nb, struct gf_dev, notifier);
	if (evdata && evdata->data &&  gf_dev) {
		blank = *(int *)(evdata->data);
		...
		unsigned long event, void *data)
{
	...
	pr_info("[info] %s go to the goodix_fb_state_chg_callback value = %d\n",
			__func__, (int)event);
	gf_dev = container_of(nb, struct gf_dev, notifier);
	if (evdata && evdata->data && event == FB_EARLY_EVENT_BLANK && gf_dev) {
		blank = *(int *)(evdata->data);
		switch (blank) {
...
#if defined(CONFIG_DRM)
#if defined(CONFIG_DRM_PANEL)
	.notifier_call = goodix_panel_notifier_callback,
#else
...
/*Modefied by weiqiang.zhou ,FR  10301608 ,on 2020.11.10 End*/
...
#endif
{
	...
	for (i = 0; i < ARRAY_SIZE(maps); i++) {
        pr_info("input type = %d,code value = %d", maps[i].type,maps[i].code);
...
#if defined(CONFIG_DRM_PANEL)
		gf_check_panel_dt(gf_dev);
        if (gf_dev->active_panel) {

            pr_info("'%s' find active_panel\n", __func__);
			err = drm_panel_notifier_register(gf_dev->active_panel, &gf_dev->notifier);
		    if (err < 0) {
                pr_info("'%s', Couldn't register DRM notifier err = %d\n", __func__ ,err);
		    }

        } else {
			pr_info("'%s' Couldn't find active_panel\n", __func__);

        }
#else
		err = msm_drm_register_client(&gf_dev->notifier);
		if (err < 0) {
            pr_info("'%s', Couldn't register DRM notifier err = %d\n", __func__ ,err);
		}
#endif
...
#else
		 gf_dev->notifier = goodix_noti_block;
		 err = fb_register_client(&gf_dev->notifier);
		if (err < 0) {

			pr_info("'%s', Couldn't register FB notifier = %d\n", __func__ ,err);
		}
#endif
...
//weiqiang.zhou
#if defined(CONFIG_DRM_PANEL)
    err = drm_panel_notifier_unregister(gf_dev->active_panel,&gf_dev->notifier);
#elif
    err = msm_drm_unregister_client(&gf_dev->notifier);
#endif
//#elif defined(CONFIG_FB)
#else
    err = fb_unregister_client(&gf_dev->notifier);
#endif
/*Modefied by weiqiang.zhou ,FR  10301608 ,on 2020.11.10 end*/
	mutex_unlock(&device_list_lock);
	pr_info("exit gf_remove.\n");
...
static int __init gf_init(void)
{
    ...
    pr_info("zwq ---gf register platform driver end\n");
#elif defined(USE_SPI_BUS)
    pr_info("zwq ---gf register SPI driver\n");
