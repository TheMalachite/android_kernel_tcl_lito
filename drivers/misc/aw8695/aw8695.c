...
#include <linux/init.h> // MODIFIED by hongwei.tian, 2020-03-02,BUG-8956218
#include <linux/i2c.h>
...
/* MODIFIED-BEGIN by hongwei.tian, 2020-03-02,BUG-8956218*/
...
/* MODIFIED-BEGIN by hongwei.tian, 2020-02-24,BUG-8821781*/
#ifdef AW_UEFI_CAL_F0
unsigned int aw8695_f0_cal = 0;
#endif
/* MODIFIED-END by hongwei.tian,BUG-8821781*/
...
static int aw8695_rtp_osc_calibration(struct aw8695 *aw8695)
{
	...
	if(aw8695->rtp_cnt == aw8695_rtp->len) {
        ...
        if (aw8695->microsecond > (aw8695->theory_time + (aw8695->theory_time / 20))) {
            pr_info("%s over time out aw8695->rtp_cnt %d osc_int_state %02x\n", __func__,aw8695->rtp_cnt, osc_int_state);
            break;
          }
    }
...
//early_param("haptic_f0:f0=",aw8695_get_f0_cmd);
...
/* MODIFIED-END by hongwei.tian,BUG-8821781*/
 ...
 //   aw8695_haptic_trig_param_config(aw8695); // MODIFIED by hongwei.tian, 2020-02-24,BUG-8821781
...
static irqreturn_t aw8695_irq(int irq, void *data)
{
    ...
    if(reg_val & AW8695_BIT_SYSINT_FF_AEI) {
        printk("%s: aw8695 rtp fifo almost empty int\n", __func__);
        if(aw8695->rtp_init) {
            while((!aw8695_haptic_rtp_get_fifo_afi(aw8695)) &&
                    (aw8695->play_mode == AW8695_HAPTIC_RTP_MODE)) {
                mutex_lock(&aw8695->rtp_lock);
                printk("%s: aw8695 rtp mode fifo update, cnt=%d\n",
                        __func__, aw8695->rtp_cnt);
                if (!aw8695_rtp) {
                   pr_info("%s:aw8695_rtp is null break\n",
                        __func__);
            ...
            printk("%s: aw8695 rtp init = %d, init error\n", __func__, aw8695->rtp_init);
        }
    }

    if(reg_val & AW8695_BIT_SYSINT_FF_AFI) {
        printk("%s: aw8695 rtp mode fifo full empty\n", __func__);
    }
...
static int aw8695_i2c_probe(struct i2c_client *i2c, const struct i2c_device_id *id)
{
    ...
    if (ret < 0) {
        ...
        i2c_check_status_create("viber_aw8695",0); // MODIFIED by hongwei.tian, 2020-01-20,BUG-8770360
        goto err_id;
    }
    ...
    i2c_check_status_create("viber_aw8695",1); // MODIFIED by hongwei.tian, 2020-01-20,BUG-8770360
    return 0;
...
/* MODIFIED-BEGIN by hongwei.tian, 2020-03-28,BUG-9073038*/
#ifdef CONFIG_PM_SLEEP
        .pm = &aw8695_pm_ops,
#endif
/* MODIFIED-END by hongwei.tian,BUG-9073038*/
    },
