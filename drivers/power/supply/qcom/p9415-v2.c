...
#define DCIN_VALID_MIN_VOUT_UV	(1000000)
#define DCIN_VALID_MAX_VOUT_GAPS	(100000)

#define DCIN_ALLOW_MIN_VOL_UV    (5000000)
#define DCIN_ALLOW_MAX_VOL_UV    (12000000)

#define DCIN_BPP_VOL_UV    (5500000)
#define DCIN_EPP_VOL_UV    (11000000)

#define DCIN_5W	(5)
#define DCIN_10W	(10)
#define DCIN_15W	(15)

#define DCIN_UV_ICL_UA   (500000)

#define DCIN_5W_BPP_ICL_UA    (900000)
#define DCIN_5W_BPP_FCC_UA    (1000000)

#define DCIN_5W_EPP_ICL_UA    (500000)
#define DCIN_5W_EPP_FCC_UA    (1000000)

#define DCIN_7P5W_EPP_ICL_UA    (600000)
#define DCIN_7P5W_EPP_FCC_UA    (1500000)

#define DCIN_10W_EPP_ICL_UA    (800000)
#define DCIN_10W_EPP_FCC_UA    (2000000)

#define DCIN_15W_EPP_ICL_UA    (1100000)
#define DCIN_15W_EPP_FCC_UA    (2400000)

#define DCIN_OT_ICL_UA	(500000)
...
struct p9415 {
	...
	struct votable		*dcin_vout_votable;
...
static int p9415_get_prop_voltage_now(struct p9415 *chip, int *pval)
{
	...
	*pval = P9415_RAW_TO_VOUT_UV(raw);
	if ((*pval < 0) || (*pval > DCIN_ALLOW_MAX_VOL_UV)) {
		p9415_dbg(DBG_ERR,
...
static int p9415_set_prop_vout_target(struct p9415 *chip, int uV)
{
	int rc;
	u16 raw;
...
static int dcin_vout_vote_callback(struct votable *votable, void *data,
			int vout_uv, const char *client)
{
	struct p9415 *chip = data;

	if ((client == NULL) || (vout_uv < 0)) {
		return -EINVAL;
	}

	p9415_dbg(DBG_DBG, "set dcin vout to %d\n", vout_uv);
	return p9415_set_prop_vout_target(chip, vout_uv);
}
...
static int __maybe_unused p9415_set_tx_mode(struct p9415 *chip, int enable)
{
	int rc = 0, retry_times = 50;
	u8 status = 0;
	...
	if (last_enable == enable) {
		p9415_dbg(DBG_DBG, "skip for same %d\n", enable);
		return 0;
	}
	...
	if (enable) {
		...
		/* wait until TX_INIT_INT bit7 of reg0x2C changed to 1 */
		while (retry_times-- > 0) {
			rc = p9415_read_byte(chip, P9415_STATUS_TRX_REG, &status);
			if (rc) {
				p9415_dbg(DBG_ERR,
						"read isr status reg error, rc=%d\n", rc);
				goto out;
			} else {
				if (status & P9415_STATUS_TX_TXINIT_BIT) {
					rc = p9415_write_mask_byte(chip,
								P9415_TX_SCR_REG,
								P9415_SCR_TX_EN_BIT,
								P9415_SCR_TX_EN_BIT);
					if (rc) {
						p9415_dbg(DBG_ERR, "enable tx error, rc=%d\n", rc);
						goto out;
					}
					p9415_dbg(DBG_DBG, "tx init done: %02x\n", status);
					last_enable = 1;
					return 0;
				}
				msleep(10);
		...
		rc = -ETIME;
		p9415_dbg(DBG_ERR, "tx init timeout: %02x\n", status);
	}
		...
		union power_supply_propval *pval)
{
	...
	case POWER_SUPPLY_PROP_VOLTAGE_MAX:
		*val = get_effective_result_locked(chip->dcin_vout_votable);
		if (*val < DCIN_BPP_VOL_UV) {
			*val = chip->target_vout;
		}
		break;
		...
		const union power_supply_propval *pval)
{
	...
	if (prop == POWER_SUPPLY_PROP_DC_START_TX) {
		if (!chip->support_tx_mode) {
			p9415_dbg(DBG_ERR, "board can't support tx ops\n");
			return -EINVAL;
		}
		mutex_lock(&chip->mtp_lock);
		if (!atomic_read(&chip->dc_present) && pval->intval
			&& !enable_tx && !chip->tx_started) {
			p9415_dbg(DBG_ERR, "user enabling tx mode\n");
			enable_tx = 1;
		} else if (!pval->intval && enable_tx
					&& chip->tx_started) {
			p9415_dbg(DBG_ERR, "user disabling tx mode\n");
			enable_tx = 0;
		} else {
			rc = -EBUSY;
			p9415_dbg(DBG_ERR, "tx/rx ops busy\n");
		}
		mutex_unlock(&chip->mtp_lock);
		if (!rc && chip->dc_psy)
			power_supply_changed(chip->dc_psy);
...
static int p9415_parse_dt(struct p9415 *chip)
{
	...
	chip->support_tx_mode =
		of_property_read_bool(node, "idt,support-tx-mode");

	rc = of_property_read_u32(node, "idt,rx-max-power",
					&chip->rx_max_power);
	if ((rc < 0) || (chip->rx_max_power > DCIN_15W)
		|| (chip->rx_max_power < DCIN_5W)) {
...
/* Warning: for older hw like Ottawa board, it need USB port
to power on the Vout line, which can do mtp ops.
For new hw design like Chicago board, no this limitation.
Currently, we use support_tx_mode to distinguish the different
boards design between old and new DUTs. 
*/
...
static void p9415_adjust_rx_power(struct p9415 *chip)
{
	...
	vout = get_effective_result(chip->dcin_vout_votable);
	if (vout < DCIN_BPP_VOL_UV) {
		vout = chip->target_vout;
	}

	if ((chip->vout > DCIN_VALID_MIN_VOUT_UV) &&
		(chip->vout < (vout - DCIN_VALID_MAX_VOUT_GAPS))) {
	...
	if ((chip->real_soc != soc || chip->vout >= 4000000)
		&& ((dc_status & 0xC0) == 0x40)) {
	...
	if (!chip->hot_occured && (quiet_temp >= lvl1)) {
		chip->hot_occured = true;
		vote(chip->dcin_icl_votable, DCIN_OT_VOTER,
			true, DCIN_OT_ICL_UA);
		vote(chip->dcin_vout_votable, DCIN_OT_VOTER,
			true, DCIN_BPP_VOL_UV);
	}
	...
	p9415_dbg(DBG_ERR,"[%d,%d,%d,%d,%d,%x,%d,%d,%d,%d]:%d,%d,%d,%d\n",
			chip->tx_watt, quiet_temp, soc, vbat, fcc, dc_status,
			chip->vout_uv_cnt, chip->vout_uv_err, chip->vout_uv_ua, step_icl_ua,
			chip->vout, iout, get_effective_result_locked(chip->dcin_icl_votable),
...
static int p9415_init_rx_hw(struct p9415 *chip)
{
	...
	if (chip->mode & P9415_MODE_EPP_BIT) {
		...
		if (chip->tx_watt < DCIN_10W) {
			chip->target_icl = DCIN_5W_EPP_ICL_UA;
			chip->target_fcc = DCIN_5W_EPP_FCC_UA;
		} else if (chip->tx_watt < DCIN_15W) {
			if (!chip->pg_invalid) {
				chip->target_icl = DCIN_10W_EPP_ICL_UA;
				chip->target_fcc = DCIN_10W_EPP_FCC_UA;
			} else {
				chip->target_icl = DCIN_7P5W_EPP_ICL_UA;
				chip->target_fcc = DCIN_7P5W_EPP_FCC_UA;
			}
		} else {
			if (!chip->pg_invalid) {
				chip->target_icl = DCIN_15W_EPP_ICL_UA;
				chip->target_fcc = DCIN_15W_EPP_FCC_UA;
			} else {
				chip->target_icl = DCIN_10W_EPP_ICL_UA;
				chip->target_fcc = DCIN_10W_EPP_FCC_UA;
			}
		}
		chip->target_vout = DCIN_EPP_VOL_UV;
	} else {
		chip->tx_watt = DCIN_5W;
		chip->target_fcc = DCIN_5W_BPP_FCC_UA;
		chip->target_vout = DCIN_BPP_VOL_UV;

		if (chip->pg_invalid) {
			chip->target_icl = DCIN_UV_ICL_UA;
		} else {
			chip->target_icl = DCIN_5W_BPP_ICL_UA;
		}
	}
...
static void p9415_reset_current(struct p9415 *chip)
{
	...
	chip->int_irq_cnt = 0;
	chip->vout_uv_cnt = 0;
	chip->hot_occured = false;
	chip->warm_occured = false;
}
...
static void p9415_status_change_work(struct work_struct *work)
{
	...
	mutex_lock(&chip->mtp_lock);
	if (mtp_start > 0) {
		// let tx -> mtp -> tx mode auto changed.
		p9415_set_vout_power(chip, false);
		chip->tx_started = 0;
	...
	/* TX mode enable part coding */
	if (chip->support_tx_mode) {
		mutex_lock(&chip->mtp_lock);
		if (!chip->tx_started && enable_tx) {
			rc = p9415_set_tx_mode(chip, 1);
			if (rc) {
				goto exit_tx_mode;
			}
			chip->tx_started = true;
		} else if (chip->tx_started && !enable_tx) {
			p9415_dbg(DBG_ERR, "user force to rx\n");
			goto exit_tx_mode;
		}

		if (chip->tx_started) {
			rc = p9415_read_ssr(chip, &ssr);
			if (rc) {
				goto exit_tx_mode;
			}
			p9415_dbg(DBG_DBG, "tx ssr: %x\n", ssr);
			if (ssr & P9415_STATUS_TX_GETDPING_BIT) {
				p9415_dbg(DBG_ERR, "tx conflict: %x\n", ssr);
				goto exit_tx_mode;
			}
			...
			p9415_dbg(DBG_DBG, "current mode: %x\n", chip->mode);
			if ((chip->mode & P9415_MODE_TX_BIT)
				== P9415_MODE_TX_BIT) {
				p9415_dbg(DBG_ERR, "stay in tx mode: %x\n",
							chip->mode);
				mutex_unlock(&chip->mtp_lock);
				goto out;
			}
...
exit_tx_mode:
		p9415_set_tx_mode(chip, 0);
		enable_tx = 0;
		chip->tx_started = false;
		mutex_unlock(&chip->mtp_lock);
	}
...
static int p9415_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	chip->dcin_vout_votable = create_votable("DCIN_VOUT", VOTE_MIN,
					dcin_vout_vote_callback, chip);
	if (IS_ERR(chip->dcin_vout_votable)) {
		p9415_dbg(DBG_ERR, "Couldn't create dcin_vout_votable\n");
		rc = PTR_ERR(chip->dcin_vout_votable);
		chip->dcin_vout_votable = NULL;
		goto cleanup;
	}
	...
	dev_info(chip->dev, "probe HW ver2 done with %d\n",
			atomic_read(&chip->dc_present));
	return 0;

cleanup:
	if (chip->dcin_vout_votable) {
		destroy_votable(chip->dcin_vout_votable);
		chip->dcin_vout_votable = NULL;
	}
...
MODULE_DESCRIPTION("P9415 driver ver2");
