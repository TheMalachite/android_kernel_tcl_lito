...
extern void tfa98xx_sysfs_init(struct tfa98xx *tfa98xx);
extern void tfa98xx_rec_sysfs_init(struct tfa98xx *tfa98xx);
                               ...
                               struct snd_ctl_elem_value *ucontrol)
{
    ...
    ucontrol->value.integer.value[0] = selector;
	mutex_unlock(&tfa98xx_mutex);
                ...
                int num_dai)
{
    ...
    /* MODIFIED-BEGIN by hongwei.tian, 2020-01-15,BUG-8822111*/
    pr_debug("%s num_dai [%d]: num_widgets [%d]\n",__func__ ,num_dai, num_widgets);

    if (dai_drv && num_dai > 0)
        for(i = 0; i < num_dai; i++) {
#ifdef CONFIG_SND_SMARTPA_COMPATIBLE
            snprintf(buf, 50, "%s","compatible-aif");
#else
            snprintf(buf, 50, "%s-%x-%x",dai_drv[i].name, i2cbus,
                addr);
#endif
/* MODIFIED-END by hongwei.tian,BUG-8822111*/
...
/*cwying 2020/7/23 add for debbug reg */
...
/*cwying 2020/7/23 add for debbug reg end*/
static void tfa98xx_dsp_init(struct tfa98xx *tfa98xx)
{
    ...
    if (tfa98xx->init_count < TF98XX_MAX_DSP_START_TRY_COUNT) {
        ...
        if (ret == Tfa98xx_Error_Not_Supported) {
			...
			tfa98xx_do_regdump(tfa98xx);//wanying.chen add for debug
            reschedule = true;
        } else {
            sync = true;
...
static int tfa98xx_mute(struct snd_soc_dai *dai, int mute, int stream)
{
    ...
    if (mute) {
		...
		{
			tfa98xx->pstream = 1;
			pr_err("%s TFA98XX_FLAG_CHIP_SELECTED:%d TFA98XX_DSP_INIT_PENDING :%d \n",__func__,
				TFA98XX_FLAG_CHIP_SELECTED,TFA98XX_DSP_INIT_PENDING);
			pr_err("%s tfa98xx->flags:%d tfa98xx->dsp_init :%d \n",__func__,
				(tfa98xx->flags & TFA98XX_FLAG_CHIP_SELECTED),(tfa98xx->dsp_init != TFA98XX_DSP_INIT_PENDING));
                	/* Start DSP */
                	if ((tfa98xx->flags & TFA98XX_FLAG_CHIP_SELECTED) &&
			   			(tfa98xx->dsp_init != TFA98XX_DSP_INIT_PENDING))
                        	tfa98xx_dsp_init(tfa98xx);
...
static struct snd_soc_dai_driver tfa98xx_dai[] = {
    {
        ...
        .symmetric_channels = 0, // MODIFIED by hongwei.tian, 2020-02-24,BUG-8951809
...
static int tfa98xx_parse_dt(struct device *dev, struct tfa98xx *tfa98xx,
	...
	tfa98xx->pa_flag =  of_property_read_bool(np, "pa_flag");
	if (tfa98xx->pa_flag < 0)
		dev_dbg(dev, "No pa_flag provided.\n");
	//pr_info("tfa98xx->pa_flag=%d\n", tfa98xx->pa_flag);
...
/* MODIFIED-BEGIN by hongwei.tian, 2020-01-15,BUG-8822111*/
#ifdef CONFIG_SND_SMARTPA_COMPATIBLE
unsigned int compatible_smartpa_find = 0;
#endif
...
/* MODIFIED-BEGIN by hongwei.tian, 2019-05-10,BUG-7754996*/
	...
	if(tfa98xx->pa_flag )
	tfa98xx_sysfs_init(tfa98xx);
	else  
	tfa98xx_rec_sysfs_init(tfa98xx);
#endif
/* MODIFIED-END by hongwei.tian,BUG-7754996*/
	...
	i2c_check_status_create("audio_rec_smpart_pa",1); // MODIFIED by hongwei.tian, 2020-01-20,BUG-8770360
	#endif
    return 0;
}
