...
#define AWINIC_CALI_FILE  "/oempersist/audio/aw_cali.bin" //modify  by lanying.he for XR10016042 on 2020/09/29
...
static int aw881xx_write_cali_re_to_file(uint32_t cali_re, uint8_t channel)
{
	...
	fp = filp_open(AWINIC_CALI_FILE, O_RDWR | O_CREAT, 0664); //modify  by lanying.he for XR10016042 on 2020/09/29
	if (IS_ERR(fp)) {
...
static int aw881xx_get_cali_re_from_file(uint32_t *cali_re, uint8_t channel)
{
	...
	if (IS_ERR(fp)) {
		...
		*cali_re = ((cali_re_default << 12) / 1000);
		aw881xx_write_cali_re_to_file((*cali_re),0);
		return -EINVAL;
	}
...
void aw881xx_get_cali_re(struct aw881xx_cali_attr *cali_attr)
{
	...
	//End  by lanying.he for XR10016042 on 2020/09/29
}
				...
				const char *buf, size_t count)
{
    ...
    pr_info("%s : cali_re = %d \n ",__func__,cali_attr->cali_re); //added  by lanying.he for XR10016042 on 2020/09/29
