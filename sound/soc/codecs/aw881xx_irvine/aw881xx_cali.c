...
#define AWINIC_CALI_FILE  "/oempersist/audio/aw_cali.bin" //modify by lanying.he for XR9824828 on 2020-08-31
...
static int aw881xx_write_cali_re_to_file(uint32_t cali_re, char *name_suffix)
{
	...
	fp = filp_open(AWINIC_CALI_FILE, O_RDWR | O_CREAT, 0664); //modify by lanying.he for XR9824828 on 2020-08-31
	if (IS_ERR(fp)) {
				...
				const char *buf, size_t count)
{
	...
	pr_info("%s : cali_re = %d \n ",__func__,cali_attr->cali_re);   //added by lanying.he for XR9824828 on 2020-08-31
				...
				size_t count)
{
	...
	int ret = -1;
	unsigned int databuf[2] = { 0 };
    //Begin modify by lanying.he for XR9824828 on 2020-08-31
    pr_info("%s : \n ",__func__);
	ret = kstrtouint(buf, 0, &databuf[0]);
	if (ret < 0)
		return ret;
